...
def test_active_lane_shows_label_phase_and_streamed_tail():
    lanes = Lanes("链接", total=3, status_labels={"summary": "摘要"})
    lanes.start("a", "Alpha")
    lanes.phase("a", "摘要")
...
def test_done_advances_count_and_tally_and_drops_active_lane():
    lanes = Lanes("x", total=2, status_labels={"summary": "摘要"})
    lanes.start("a", "Alpha")
...
def test_failure_persists_after_more_work():
    lanes = Lanes("x", total=3, status_labels={"failed": "失败"})

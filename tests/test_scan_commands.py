...
def _make_synth_db(tmp_path, rows: list[tuple]) -> sqlite3.Connection:
    ...
    *rows* is a list of ``(create_time, local_type, sender_wxid, content_bytes)``.
    ...
    for ct, lt, wxid, content in rows:
        conn.execute(
            f"INSERT INTO {GROUP_TABLE} (create_time, local_type, real_sender_id, message_content) "
            f"VALUES (?, ?, ?, ?)",
...
def test_scan_commands_sets_alias(patched_aliases, monkeypatch, tmp_path):
    mod = patched_aliases
    rows = [(1000, MSG_TEXT, "wxid_alice", _prefixed("wxid_alice", "/alias Duckie"))]
...
def test_scan_commands_sees_owner_prefixless_command(patched_aliases, monkeypatch, tmp_path):
    ...
    mod = patched_aliases
    rows = [(1000, MSG_TEXT, "wxid_owner", _bare("/alias Boss"))]
    conn = _make_synth_db(tmp_path, rows)
    ...
    db.scan_commands(ContactMap.from_dict({"wxid_owner": "Owner"}))
...
def test_scan_commands_writes_cursor(patched_aliases, monkeypatch, tmp_path):
    mod = patched_aliases
    rows = [
        (1000, MSG_TEXT, "wxid_alice", _prefixed("wxid_alice", "/alias Duckie")),
        (2000, MSG_TEXT, "wxid_alice", _prefixed("wxid_alice", "hello world")),  # non-command
    ]
    conn = _make_synth_db(tmp_path, rows)
    monkeypatch.setattr(mod, "get_conn", lambda rel: conn if "message_0" in rel else (_ for _ in ()).throw(FileNotFoundError()))
...
def test_scan_commands_incremental(patched_aliases, monkeypatch, tmp_path):
    ...
    rows = [
        (1000, MSG_TEXT, "wxid_alice", _prefixed("wxid_alice", "/alias Duckie")),
        (2000, MSG_TEXT, "wxid_bob", _prefixed("wxid_bob", "/alias Quackie")),
    ]
    conn = _make_synth_db(tmp_path, rows)
    monkeypatch.setattr(mod, "get_conn", lambda rel: conn if "message_0" in rel else (_ for _ in ()).throw(FileNotFoundError()))
...
def test_scan_commands_ignores_non_slash_text(patched_aliases, monkeypatch, tmp_path):
    mod = patched_aliases
    rows = [
        (1000, MSG_TEXT, "wxid_alice", _prefixed("wxid_alice", "hello there, /alias Duckie")),
        # Command not on first line should be ignored too
        (2000, MSG_TEXT, "wxid_bob", _prefixed("wxid_bob", "hi\n/alias Duckie")),
    ]
    conn = _make_synth_db(tmp_path, rows)
    monkeypatch.setattr(mod, "get_conn", lambda rel: conn if "message_0" in rel else (_ for _ in ()).throw(FileNotFoundError()))
...
def test_scan_commands_optout_then_optin(patched_aliases, monkeypatch, tmp_path):
    mod = patched_aliases
    rows = [
        (1000, MSG_TEXT, "wxid_alice", _prefixed("wxid_alice", "/optout")),
        (2000, MSG_TEXT, "wxid_alice", _prefixed("wxid_alice", "/optin")),
    ]
    conn = _make_synth_db(tmp_path, rows)
    monkeypatch.setattr(mod, "get_conn", lambda rel: conn if "message_0" in rel else (_ for _ in ()).throw(FileNotFoundError()))

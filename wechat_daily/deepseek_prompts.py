...
**二级章节默认就用四个标准大类**：`## 行业新闻`、`## 工具`、`## 方法论`、`## 闲聊花絮`（没有的就不写）。**专题是例外，不是常态**——只有当某个单一话题当天占讨论量约 1/3 以上、且自身能拆出 3 个以上子话题时，才可为它单独抽一个 `##` 专题章节（4–10 字短名词短语）。不要把零散话题硬凑成专题，也不要给每个大类都套上花哨标题；多数日子应该是"三四个标准大类 + 至多一个专题"的样子。`## 闲聊花絮` 永远用此固定名、放最后、不抽专题。

**硬约束**：导读要点里点到的每条，章节树里必须有对应的 `###`；章节树里的每个 `###` 也都应是真实成立的话题。

只输出大纲（A + B 两部分），不要写任何正文段落。
"""

_WRITE_TASK_HEAD = """\
# 你的任务：按下面这份【选题大纲】写出今天的完整日报正文

下面是你上一步已经定好的大纲。**严格照它的章节树写**：

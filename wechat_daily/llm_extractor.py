...
from .config import CLAUDE_MODEL, GEMINI_SUMMARY_MODEL, DEBUG_DIR, debug_dir_for
...
def _build_extract_user_content(
    ...
    *debug_text* is always the flat string used for the debug sidecar.
    ...
    prefix = "".join(parts)
    # Default = Opus instructions (unchanged). The DeepSeek two-step path passes
    # its own already-formatted instruction block (outline pass / write pass).
    instr = instructions if instructions is not None else _USER_INSTRUCTIONS.format(date_str=date_str)
    suffix = "\n</chat_log>\n\n---\n\n" + instr
    if image_caption_mode:
        # Text-only path: images are not inlined; they appear as `[图片：…]`
        # where the description is machine-generated by a vision model. Override
        # the shared 「关于图片」 wording (which assumes the image follows inline).
        suffix += (
            "\n\n（图片说明：本次聊天记录里的图片以 `[图片：……]` 形式给出，"
            "其中冒号后的文字是视觉模型对该图的描述，并非群友原话。请据此理解"
            "图片所涉的文章、产品、截图等内容；描述已按隐私规则脱敏，引用时仍"
            "用自然语言概括，不要照搬为发言。无有效描述的图片可不写。）"
        )
...
def extract_report_deepseek(
    ...
    *,
    ...
    """Stream a markdown daily report from DeepSeek via a two-step pipeline.

    DeepSeek V4 Pro is *not* given the Opus ``_SYSTEM_PROMPT`` (which carries a
    few-shot example that hurts reasoning models). Instead it runs the
    DeepSeek-tuned prompts in :mod:`wechat_daily.deepseek_prompts`:

      1. **OUTLINE** — enumerate every candidate topic with attribution, dedup
         vs prior reports, decide keep / merge / drop, and lay out the section
         tree. This forces the planning DeepSeek otherwise skips (observed
         ~660 chars of CoT vs Opus's ~33k), which is what caused dropped topics
         and intro↔body mismatches.
      2. **WRITE** — write the full report against that outline.

    Both passes run at *temperature* 0.6 (DeepSeek's recommended reasoning
    sweet spot; the old single call inherited the client default of 1.0, which
    fed fabrication). Text-only input: when *image_caption_mode* is set the
    caller has pre-rendered images as ``[图片：<caption>]`` (see
    ``image_captioner``); otherwise they appear as bare ``[图片]``.

    Callback contract matches :func:`extract_report` so the CLI's live view and
    cost logging work unchanged — the OUTLINE pass streams into the *thinking*
    pane and the WRITE pass into the *body* pane. ``usage_cb`` fires once per
    underlying API call (outline + write) so cost tracking sees both.
    *debug_suffix* keeps sidecars from colliding with the canonical run.
    Raises :class:`ExtractionError` on refusal / truncation / empty.
    """
    from . import deepseek_client
    from .config import get_deepseek_key
    from .deepseek_prompts import (
        OUTLINE_SYSTEM, WRITE_SYSTEM,
        build_outline_instructions, build_write_instructions,
    )
    ...
    def _build(instructions: str) -> tuple[str, str]:
        return _build_extract_user_content(
            date_str=date_str,
    ...
    for attempt in range(1, max_retries + 1):
        ...
        # ── Pass 1: OUTLINE — streams into the thinking pane (it's the plan) ──
        outline_user, outline_debug = _build(build_outline_instructions(date_str))

        def on_outline(delta: str, _st=st, _attempt=attempt) -> None:
        ...
        if header_cb:
            header_cb("body", 0, "📋 选题大纲（DeepSeek 第 1 步）", attempt)

        try:
            outline, outline_reasoning, outline_usage, outline_finish = (
                deepseek_client.stream_chat(
                    ...
                    content_cb=on_outline,
                    reasoning_cb=on_outline,
                )
            )
            ...
            if attempt < max_retries:
                time.sleep(5 * attempt)
                continue
            _save_failure(date_str, outline_debug, None, f"大纲阶段失败：{e}", suffix=debug_suffix)
            raise ExtractionError(f"DeepSeek 大纲阶段失败：{e}") from e

        outline = outline.strip()
        if outline:
            _save_outline(date_str, outline, outline_reasoning, suffix=debug_suffix)
        if usage_cb and outline_usage:
            usage_cb(outline_usage, len(outline_debug))

        # An empty/truncated outline is non-fatal: fall through to WRITE with
        # whatever we got (the write prompt degrades to a normal single pass).

        # ── Pass 2: WRITE — streams into the body pane ───────────────────────
        write_user, write_debug = _build(build_write_instructions(date_str, outline))
        ...
        try:
            ...
            if attempt < max_retries:
                time.sleep(5 * attempt)
                continue
            _save_failure(date_str, write_debug, None, str(e), suffix=debug_suffix)
            raise ExtractionError(f"DeepSeek 调用失败：{e}") from e

        markdown = deepseek_client.strip_markdown_fence(content)

        if finish_reason == "length":
            _save_failure(date_str, write_debug, markdown, "响应被 max_tokens 截断", suffix=debug_suffix)
            raise ExtractionError("DeepSeek 响应被 max_tokens 截断，请增大 max_tokens 后重试")

        if not markdown.strip():
            _save_failure(date_str, write_debug, markdown, "响应为空", suffix=debug_suffix)
            raise ExtractionError("DeepSeek 响应为空")

        _save_extract(date_str, markdown, write_debug, reasoning, suffix=debug_suffix)
        if usage_cb:
            usage_cb(usage, len(write_debug))
        return DailyReport(date=date_str, markdown=markdown)

    _save_failure(date_str, chat_history, None, str(last_exc), suffix=debug_suffix)
    raise last_exc  # type: ignore[misc]
...
def _save_outline(
    ...
    """Save the DeepSeek two-step OUTLINE pass to ``debug/{date}/`` for inspection."""
    d = debug_dir_for(date_str)
    d.mkdir(exist_ok=True, parents=True)
    (d / f"extract{suffix}.outline.md").write_text(outline, encoding="utf-8")
    if reasoning:
        (d / f"extract{suffix}.outline.thinking.md").write_text(reasoning, encoding="utf-8")

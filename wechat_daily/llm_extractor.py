...
def extract_report_deepseek(
    ...
    *,
    ...
    Text-only input: DeepSeek has no vision, so images can't be inlined. When
    *image_caption_mode* is set, the caller has pre-rendered them as
    ``[图片：<caption>]`` (Gemini descriptions, see ``image_captioner``) so the
    image content still reaches the model as text; otherwise they appear as
    bare ``[图片]`` placeholders.

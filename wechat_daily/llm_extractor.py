    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            with client.messages.stream(
                model=model,
                max_tokens=128000,
                # display=summarized so the live thinking pane has content: on
                # Opus 4.6 that's already the default (no-op), but Fable 5 / Opus
                # 4.7+ default to "omitted" and would stream empty thinking deltas.
                # display affects visibility only — thinking is billed the same.
            ...
            if response.stop_reason == "refusal":
                _save_failure(date_str, debug_text, markdown, "Claude 拒绝处理该内容", suffix=debug_suffix)
            ...
            _save_extract(date_str, markdown, debug_text, thinking_text, suffix=debug_suffix)
            if usage_cb:

...
2. **二级章节**：默认从下面四类中按当天内容选用，没有的就不写。顺序默认为：专题（如有）→ `## 行业新闻` → `## 工具` → `## 方法论` → `## 闲聊花絮`；仅当某章节当天分量明显更重时才调整（`## 闲聊花絮` 恒为最末）：
   ...
   **专题章节**：当天若有一条明显的主线（一场发布、一个争议、一个贯穿全天的事件），默认把它抽为 `##` 专题章节并放在开篇，吸收围绕它的各个子话题。判断标准是「它是不是当天讨论的重心」，不必计算占比；这个群的主线常常延续数天（如某模型发布后的连续实测），续日照常抽专题，标题体现当日新角度即可。没有明显主线时退回默认四类。例：
...
数量不设配额，唯一的判断标准是**每条引用都要承担新信息**——资讯转发常常一条都不需要，高密度多人辩论值得多引。仅有一条数量红线：单个子话题的 blockquote 不超过 8 条，防止极端情况整段倒灌聊天记录。行内摘抄同理：简介里已转述过的意思不再重复摘抄，摘抄过密时优先改转述。
...
导读是取舍，不是全文缩写：点出当天最重要的几条主线、说清为什么值得读即可，细节和引语留给正文。短而有判断，好过面面俱到。

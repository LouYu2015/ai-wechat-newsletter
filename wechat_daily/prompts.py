...
1. **导读**：一个自然段。具体写法见用户消息末尾的「导读要求」。不要写 `[TOC]`。
...
数量不设配额，唯一的判断标准是**每条引用都要承担新信息**——资讯转发常常一条都不需要，高密度多人辩论值得多引。仅有一条数量红线：单个子话题的 blockquote 不超过 8 条，防止极端情况整段倒灌聊天记录。行内摘抄同理：简介里已转述过的意思不再重复摘抄，摘抄过密时优先改转述。多人对同一件事的**接龙式反应**（一人一句排开）不要逐个摘抄：收敛为一句转述整体反应，至多保留其中最传神的 1–2 句原话。
...
**形式硬约束：导读只有一个自然段，只写当天最值得读的 2–3 条线索。**选不进这 2–3 条的内容在导读里一概不提——不点名、不列举、不用「还有……」「彩蛋是……」扫尾；读者靠章节标题自己就能发现其余内容。细节和引语留给正文。短而有判断，好过面面俱到。

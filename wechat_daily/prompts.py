...
基本判断标准是**每条引用都要承担新信息**——资讯转发常常一条都不需要，高密度多人辩论值得多引；简介里已转述过的意思不再重复摘抄。**数量红线有两条**：单个子话题的 blockquote 不超过 8 条，防止极端情况整段倒灌聊天记录；**单个子话题的行内「」摘抄不超过 4 处**，超出的要么改转述、要么升为 blockquote。多人对同一件事的**接龙式反应**（一人一句排开）不要逐个摘抄：**点名不超过 3 人**（选信息量最大的），其余合并为一句整体概括（如「群友们纷纷确认额度已回满」），至多保留其中最传神的 1–2 句原话。

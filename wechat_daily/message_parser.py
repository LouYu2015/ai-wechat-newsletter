    ...
    *sender_wxid* is the authoritative sender resolved from the row's
    ...
    if local_type == MSG_TEXT:
        ...
        if not title:
            return None
        return Message(
            create_time=create_time, local_type=local_type, sender_wxid="", content=title, raw=raw
        )
        ...
        if not text:
            return None
        return Message(
            create_time=create_time, local_type=local_type, sender_wxid="", content=text, raw=raw
        )
